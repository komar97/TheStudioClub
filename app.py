...
def movement_control(kind, article_id):
    ...
    minus, value, plus = st.columns([0.55, 0.9, 0.55], gap="small")
    ...
    with value:
        st.markdown(
            f'<div class="movement-value">{int(st.session_state.get(key, 0) or 0)}</div>',
            unsafe_allow_html=True
        )
...
if st.session_state.page == "stock":
    ...
    if recherche.strip():
        articles_affiches = articles[
            articles["article"].str.contains(recherche.strip(), case=False, na=False)
        ].copy()
    else:
